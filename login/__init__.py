...
class Login:
    ...
    def get_xsrf(self):
        ...
        # 获取登录时需要用到的_xsrf
        try:
            index_page = self.__session.get(
                index_url, headers=self.headers, timeout=35)
        except:
    ...
    def check_login(self):
        ...
        try:
            login_check = self.__session.get(
                check_url, allow_redirects=False, headers=self.headers, timeout=35)
    ...
    def do_login(self):
        ...
        try:
            login_page = self.__session.post(
                post_url, postdata, headers=self.headers, timeout=35)
            print(login_page.text)
            print(postdata)
            login_json = json.loads(login_page.text.encode(
                'latin-1').decode('unicode-escape'))
            print(login_json)
            # 需要输入验证码 r = 0为登陆成功代码
